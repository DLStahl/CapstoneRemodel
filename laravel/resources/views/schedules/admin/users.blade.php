    ...
    <div class="row">
        <div class="col-md-12">
            <h5>
                ...
                <form>
                    ...
                    <input type="button" value="Add Attending" class="btn btn-success" id="Attending"
                        onclick="addUsers(this.id);">
                    <input type="button" value="Add Resident" class="btn btn-success" id="Resident"
                        onclick="addUsers(this.id);">
                </form>
            </h5>
        </div>
        <script type="text/javascript">
            function addUsers(id) {
                ...
                var url = current_url.search('/addUser/') > -1 ? current_url.substr(0, current_url.search('/addUser/')) :
                    current_url;
        ...
        <table id="users_table">
            ...
            <tbody>
                @foreach ($roles as $role)
                    <tr>
                        ...
                        <td id="button">
                            @if ($role['role'] != 'Admin')
                                <input align="center" onclick="deleteUsers(this.id);" value="Delete User" type="button"
                                    class='btn btn-md btn-success' id="{{ $role['email'] }}_{{ $role['role'] }}">
                            @else
                                <br>
                            @endif
                        </td>
            ...
            <script type="text/javascript">
                function deleteUsers(id) {
                    ...
                    // Update url to the confirmation page
                    var current_url = window.location.href;
                    var url = current_url.search('/deleteUser/') > -1 ? current_url.substr(0, current_url.search(
                        '/deleteUser/')) : current_url;
