            ...
            @foreach ($past_announcements as $past_announcement)
                @if ($past_announcement->parent_message_id == -1)
                    ...
                    <form action="announcement" method="POST">
                        ...
                        <div class="form-group">
                            <textarea id="message" name="message" class="form-control"
                                placeholder="Type your response to the above announcement or the comments below here..." /></textarea>
                        ...
                        @if ($past_announcement_response->parent_message_id == $past_announcement->id)
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    ...
                                    @endphp
                                    <p>{{ $past_announcement_response->message }}<br>{{ $submitter2 }} on
                                        {{ $past_announcement_response->created_at }}</p>
                                    <hr>
                                </div>
                                <div class="col-md-1">
                                    @if ($super_access)
                                        <form action="deleteannouncement" method="POST">
                                            <input type="submit" value="DELETE" class="btn btn-warning">
                                            <input type="hidden" id="message_id" name="message_id"
                                                value="{{ $past_announcement_response->id }}">
                                        </form>
                                    @endif
                                </div>
                            </div>
