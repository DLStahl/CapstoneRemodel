            ...
            <div class="row">
                @if (!is_null($previousChoices[0]))
                    <div class="col-12 col-md">
                        ...
                        @else
                            ...
                            <p>Milestone:<br> {{ $previousChoices[$loop->iteration - 1]['milestone'][0]['category'] }} -
                                {{ $previousChoices[$loop->iteration - 1]['milestone'][0]['detail'] }}</p>
                            <p>Objective:<br> {{ $previousChoices[$loop->iteration - 1]['objective'] }}</p>
                            @if ($previousChoices[$loop->iteration - 1]['anesthesiologist_pref'] != '[]')
                                <p>Preferred Anesthesiologist: <br>
                                    {{ $previousChoices[$loop->iteration - 1]['anesthesiologist_pref'][0]['first_name'] }}
                                    {{ $previousChoices[$loop->iteration - 1]['anesthesiologist_pref'][0]['last_name'] }}
                                </p>
                            @else
                ...
                <div class="col-12 col-md">
                    ...
                    @else
                        ...
                        <p>Milestone:<br> {{ $currentChoices[$loop->iteration - 1]['milestone'][0]['category'] }} -
                            {{ $currentChoices[$loop->iteration - 1]['milestone'][0]['detail'] }}</p>
                        <p>Objective:<br> {{ $currentChoices[$loop->iteration - 1]['objective'] }}</p>
                        @if ($currentChoices[$loop->iteration - 1]['anesthesiologist_pref'] != '[]')
                            <p>Preferred Anesthesiologist:<br>
                                {{ $currentChoices[$loop->iteration - 1]['anesthesiologist_pref'][0]['first_name'] }}
                                {{ $currentChoices[$loop->iteration - 1]['anesthesiologist_pref'][0]['last_name'] }}</p>
                            <input type="hidden" name="pref_anest{{ $loop->iteration }}"
                                value="{{ $currentChoices[$loop->iteration - 1]['anesthesiologist_pref'][0]['id'] }}">
                        @else
                            <p> Preferred Anesthesiologist:<br> No Preference </p>
                        ...
                        <input type="hidden" name="milestones{{ $loop->iteration }}"
                            value="{{ $currentChoices[$loop->iteration - 1]['milestone'][0]['id'] }}">
                        <input type="hidden" name="objectives{{ $loop->iteration }}"
                            value="{{ $currentChoices[$loop->iteration - 1]['objective'] }}">
