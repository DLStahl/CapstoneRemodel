...
@section('content')
    ...
    <div class="modal fade" id="confirmActionModal" role="dialog" aria-labelledby="confirmActionModal" aria-hidden="true"
        style="z-index: 1600;">
    ...
    <script>
        ...
        $('#editModal').on('show.bs.modal', function(event) {
            ...
            @foreach($columns as $column)
                editModal.find("#{{ $column }}-edit").val(
                    $('tr#' + id).find('td[name="{{ $column }}"]').text()
                );
            @endforeach
        });
    ...
    <button type="button" align="center" class="btn btn-md btn-success" data-toggle="modal"
    ...
    <table id="table">
        ...
        <tbody id="dataTableBody">
            @foreach ($data as $entry)
                @component('crud.databasetablerow', ['columns' => $columns, 'entry' => $entry, 'primaryKeyField' =>
                    $primaryKeyField])
                @endcomponent
            @endforeach
        </tbody>
