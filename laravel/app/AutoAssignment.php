...
class AutoAssignment
{
    public static function assignment($date)
    {
        ...
        $arrayFromSecondPref = self::assignResidentsForPref(
            $arrayFromFirstPref[0],
            2,
            2,
            $date,
            $arrayFromFirstPref[1]
        );
        //handle 3rd preference assignments with ticketsAdded = 4
        $arrayFromThirdPref = self::assignResidentsForPref(
            $arrayFromSecondPref[0],
            3,
            4,
            $date,
            $arrayFromSecondPref[1]
        );
    ...
    {
        $remainder = []; // remaining unassigned residents
        $wantedSchedules = []; //schedule ids for schedules wanted by +1 residents
        ...
        foreach ($residents as $resident) {
            ...
            } elseif (!in_array($schedulePref, $wantedSchedules)) {
