...
class UpdateScheduleData extends Command
{
    ...
    public function handle()
    {
        ...
        if ((int)Status::where('date', $date)->value('schedule') < 1) {
			...
			$allStaticChoices->each(function($choice) use ($staticDate) {
				$this->addData($staticDate, $choice->location, $choice->room, $choice->case_procedure, $choice->case_procedure_code, $choice->lead_surgeon, $choice->lead_surgeon_code, $choice->patient_class, $choice->start_time, $choice->end_time, $choice->rotation);
			});
	...
	public function addData($date, $location, $room, $case_procedure, $case_procedure_code, $lead_surgeon, $lead_surgeon_code, $patient_class, $start_time, $end_time, $rotation) {
		if (ScheduleData::where('date', $date)->where('room', $room)->doesntExist())
        {
            ...
            if (strcmp($start_time, $end_time) < 0) {
                ScheduleData::insert([
                    'date' => $date, 'location' => $location, 'room' => $room, 'case_procedure' => $case,
                    'lead_surgeon' => $surgeon, 'patient_class' => $patient_class, 'start_time' => $start_time,
                    'end_time' => $end_time, 'rotation' => $rotation
                ]);
            }
        }
	}
