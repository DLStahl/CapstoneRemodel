...
SkedTape.prototype = {
	...
	renderLocation: function(location) {
		var $text = $('<div class="sked-tape__location-text"/>')
			.text(location.name);
		var $location = $('<div class="sked-tape__location "/>')
			.attr({'title': location.name+'\n'+location.userData['time'], 'data-id': location.id})
			.append($text);
		if (window.location.href.indexOf("firstday") == -1){
			var $select = $('<select class="PreferenceSelector" id="'+location.id+'">');
			$select.append($('<option selected="selected" value= "">Choose</option>'));
			$select.append($('<option value= "1">First</option>'));
	        $select.append($('<option value= "2">Second</option>'));
	...
	renderAside: function() {
		...
		this.$locations = $('<div class="sked-tape__locations"/>')
